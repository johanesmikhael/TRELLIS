...
PLATFORM=$(python -c "import torch; print(('cuda' if torch.version.cuda else ('hip' if torch.version.hip else 'unknown')))")
case $PLATFORM in
    cuda)
...
if [ "$KAOLIN" = true ] ; then
    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        case $PYTORCH_VERSION in
            ...
            2.4.0+cu121) pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu121.html;;
            2.4.0+cu118) pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu118.html;;
